...
if (exists("population_power")) {
  msoa_sf$populations <- msoa_sf$populations / mean(msoa_sf$populations)
  msoa_sf$populations <- msoa_sf$populations ^ population_power
}
...
df <- data.frame(
  msoa_sf$id, msoa_sf[[name_property]], msoa_sf[[abbr_property]],
  msoa_sf$color, points_df$X, points_df$Y, msoa_sf$area, msoa_sf$populations)
...
tm <- tm_shape(msoa_sf) +
  ...
  tm_text(text = "ABR", legend.size.show = FALSE)
tm_view(tm)
