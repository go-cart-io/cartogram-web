...
class CartoDataFrame(gpd.GeoDataFrame):
    ...
    def __init__(self, *args, extra_attributes={}, **kwargs):
        ...
        self.is_projected = extra_attributes.get("crs", {}).get("properties", {}).get("name") == "EPSG:cartesian"
    ...
    def __setattr__(self, attr, val):
        ...
        if attr in self._metadata:
            ...
            super().__setattr__(attr, val)  # Let GeoDataFrame handle standard attributes
    ...
    def read_file(cls, filepath):
        ...
        extra_attributes = {key: value for key, value in data.items() if key != "features"}
    ...
    def to_crs(self, crs=None, epsg=None, inplace=False):
        ...
        if self.is_projected:
            warnings.warn("The file is already projected. This reprojection may be incorrect.", UserWarning)
    ...
    def clean_and_sort(self, region_col, base_columns = ['Region', 'label', 'cartogram_id', 'geometry'], prefered_names_dict = {}):
        """
        ...
        if prefered_names_dict != {}:
            self['Region'] = self['Region'].apply(lambda x: prefered_names_dict.get(x, x))

        # Identify columns to keep
        area_columns = [col for col in self.columns if col.startswith('Geographic Area')]
        ...
        # Filter columns
        self.drop(columns=[col for col in self.columns if col not in existing_columns], inplace=True)
