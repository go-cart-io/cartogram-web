...
def preprocess(input, mapDBKey='temp_filename', based_path='/tmp'):  
    ...
    cdf = cdf[cdf.geometry.type.isin(['Polygon', 'MultiPolygon'])].reset_index(drop=True)
    ...
    if not cdf.is_projected:
        ...
        cdf.to_crs("EPSG:6933", inplace=True) # NSIDC EASE-Grid 2.0 Global https://epsg.io/6933
...
def generate_cartogram(datacsv, input_file, cartogram_key, project_path, clean_by = None, print_progress = False, flags = []):
    ...
    if cdf.is_world:
        flags = flags + ['--world']
    if clean_by != None and clean_by != '':
        cdf.clean_and_sort(clean_by or 'Region', prefered_names_dict = prefered_names_dict)
        cdf.to_carto_file(input_file)

    if is_projected:
        equal_area_json = preprocess_geojson(cartogram_key, input_file, datasets[0], flags + ['--output_shifted_insets', '--skip_projection'])
    else:
        equal_area_json = preprocess_geojson(cartogram_key, input_file, datasets[0], flags + ['--output_equal_area_map'])
    ...
    for i, dataset in enumerate(datasets):
        ...
        with open(f'{project_path}/{name}_simplified.json', 'w') as outfile:
            ...
            cartogram_json_simplified = util.add_attributes(cartogram_json_simplified, is_projected = True)
...
def process_data(csv_string):
    ...
    if not 'ColorGroup' in df:
        df["ColorGroup"] = '' # Just to make sure ColorGroup column exists. Color assignment sould be done during geojson processing
...
def call_binary(params, data_index = 0, data_length = 1, print_progress = False):
    ...
    for source, line in cartwrap.generate_cartogram(area_data_path, params['gen_file'], cartogram_exec, flags):
