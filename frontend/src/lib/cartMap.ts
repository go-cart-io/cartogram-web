...
export default class CartMap {
   ...
   */
  getVersionPolygonScale(sysname: string): [number, number] {
    const version_width =
   ...
   * @param value
   */
  verifyLegend(sysname: string, squareWidth: number, valuePerSquare: number) {
  ...
  drawLegend(
    ...
    const total_value_Y = 5 + width + 20
  ...
  drawResizableLegend(sysname: string, legendSVGID: string, old_sysname: string = null) {
    ...
    const total_value_Y = 5 + widthC + 20
    totalValue.attr('y', total_value_Y.toString() + 'px')
   ...
   * @param {number} height
   */
  getGridPath(gridWidth: number, width: number, height: number) {
    let gridPath = ''
