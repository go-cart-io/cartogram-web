...
export default class Cartogram {
  ...
  launchGridEdit() {
    ...
    if (this.model.gridedit_window === null || this.model.gridedit_window.closed) {
      this.model.gridedit_window = window.open(
        ...
        'width=550,height=650,resizable,scrollbars'
      )

      this.model.gridedit_window.addEventListener(
        'load',
        function (gd) {
          return function (e) {
  ...
  drawPieChartFromTooltip(container, tooltip, colors) {
    ...
    const dataWithOthers = Object.keys(this.model.map.regions)
      .map((region_id, _i, _a) => {
        return {
          label: region_id,
    ...
    slice = slice
      ...
      .on('mouseover', function (event, d, i) {
        ...
        Tooltip.drawWithEntries(event, d.data.name, d.data.abbreviation, [
          {
            name: tooltip.label,
            value: d.data.value,
            unit: tooltip.unit
  ...
  enterLoadingState() {
    ...
    if (
      ...
      typeof this.model.gridedit_window.gridedit === 'object'
    ) {
  ...
  updateProgressBar(min, max, value) {
    if (value < max) value = Math.max(min, value)
    else value = Math.min(max, value)
    ...
    return new Promise(
      function (resolve, reject) {
        ...
        var progressUpdater = window.setInterval(
          (function (cartogram_inst, key) {
            return function () {
              HTTP.get(
                cartogram_inst.config.getprogress_url +
                  '?key=' +
          ...
          })(this, unique_sharing_key),
          500
        )

        HTTP.post(this.config.cartogram_url, req_body, {
  ...
  displayVersionSwitchButtons() {
    ...
    Object.keys(this.model.map.versions)
      .sort()
      .forEach(function (sysname) {
          ...
          yesButton.addEventListener(
            ...
            function (sysname, response) {
              return function (e) {
                ...
                this.getGeneratedCartogram(
                  sysname,
                  response.areas_string,
                  ...
                  function (cartogram) {
                    ...
                    this.model.map.drawVersion('1-conventional', 'map-area', [
                      'map-area',
                      'cartogram-area'
                    ])
