...
onBeforeMount(() => {
  ...
  if (urlParams.get('zoom') === '0') store.options.zoomable = false
  else store.options.zoomable = true
  if (urlParams.get('rotate') === '0') store.options.rotatable = false
  else store.options.rotatable = true
  if (urlParams.get('stretch') === '0') store.options.stretchable = false
  else store.options.stretchable = true
})
...
async function switchMap(newmappack: Mappack) {
  ...
  tracker.setMap(store.currentMapName)
  if (urlParams.get('pid')) {
    ;[store.options.zoomable, store.options.rotatable, store.options.stretchable] = config(
      urlParams.get('pid')!,
  ...
  tracker.setMeta(
    'features_set',
    store.options.zoomable + ':' + store.options.rotatable + ':' + store.options.stretchable
  )
